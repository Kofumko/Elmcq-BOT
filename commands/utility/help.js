export default {
    ...
    execute: async (sock, msg, from, args, db, { isAdmin, isOwner, senderNumber }) => {
        ...
        if (categoryArg === 'stats') {
            const statsHelp = `📊 *KATEGORI: STATS* 📊\n\n` +
        ...
        if (categoryArg === 'ekonomi') {
            const ekonomiHelp = `💰 *KATEGORI: EKONOMI* 💰\n\n` +
        ...
        if (categoryArg === 'game') {
            ...
            const gameHelp = `🎮 *KATEGORI: GAME CENTER* 🎮\n\n` +
        ...
        if (categoryArg === 'info') {
            const infoHelp = `ℹ️ *KATEGORI: INFO* ℹ️\n\n` +
        ...
        if (categoryArg === 'sistem') {
            ...
            const sistemHelp = `⚙️ *KATEGORI: SISTEM* ⚙️\n\n` +
        ...
        if (categoryArg === 'server') {
            if (!isGroup1) return sock.sendMessage(from, { text: '❌ *Akses Ditolak!* Kategori *Server* hanya tersedia di *Grup Pusat*.' });

            let serverHelp = `🏛️ *KATEGORI: SERVER* 🏛️\n\n` +
        ...
        if (categoryArg === 'toko') {
            const tokoHelp = `📦 *KATEGORI: TOKO & BRANKAS* 📦\n\n` +
                `*-- PUSAT BELANJA --*\n` +
        ...
        if (categoryArg === 'misi') {
            const misiHelp = `💼 *KATEGORI: JOB BOARD* 💼\n\n` +
        ...
        return sock.sendMessage(from, { text: '❌ *Kategori tidak ditemukan!* Pilih: STATS, bank, ekonomi, info, toko, misi, game, Sistem, Server.' });
    }
};
